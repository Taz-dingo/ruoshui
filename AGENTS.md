...
- ruoshui-doc-sync: Manage project docs, status sync, handoffs, engineering-memory updates, and “which doc should change” decisions for 若水广场. (file: ./.codex/skills/ruoshui-doc-sync/SKILL.md)
...
- Use `ruoshui-doc-sync` when the task is mainly about updating docs, deciding whether docs should be updated, syncing `state/tasks/engineering-memory`, or preparing a durable handoff.
...
- 涉及文档归档、状态同步、handoff、`state/tasks/engineering-memory` 取舍时，必须同时使用 `ruoshui-doc-sync`，不要只靠临场判断。
