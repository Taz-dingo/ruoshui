...
- 已补上首个 `WebGPU` 尝试链路：当前 viewer 初始化会优先请求 `WebGPU`、失败后自动回退到 `WebGL2`，并在开发态 HUD 中显示实际图形后端，便于本机直接对比同一路径下的真实收益
- 已确认“运行时俯视捕获 → 自动描边生成小地图”这条链路在当前 `PlayCanvas + SOG` 方案下不稳定，现已撤回；当前小地图重新收口为“稳定的交互覆盖层 + 预留静态底图资产入口”，后续改为接入人工确认过的独立底图资源
- 已放开渲染清晰度上限到当前设备原生像素比，并新增可选抗锯齿开关：当前稳定策略只在 `WebGL2` 回退时提供 `FXAA`，`WebGPU` 暂不再强行启用 `TAA`；同时清晰度滑块已改为 `1%` 精度，避免在非整档 `DPR` 设备上超过或达不到原生上限

## 当前一期上线缺口

- 需要继续收敛 UI 文案与信息密度，强化纪念感并去掉多余操作入口
