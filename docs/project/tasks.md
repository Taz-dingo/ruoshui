...
- [x] 完成 `web/` 样式系统重构收口：移除大部分 legacy 全局 selector，把组件视觉迁到 `TSX + Tailwind + style system primitive`
...
- [x] 增加学校位置与相机状态小地图
- [x] 收口小地图真实底图对齐链路，并恢复开发态校准面板
- [ ] 收敛首页 / 场景页文案，改成更怀念向表达
- [x] 删除当前多余的“进入 / 全览”按钮与同类冗余操作
...
- 当前第一期静态站部署方式已更新为：`Cloudflare Pages + R2`，页面壳子走 `Pages`，超限生产主模型走 `R2` 公网地址
...
- 最近一轮“开启抗锯齿后像马赛克、但拖一下清晰度滑块就恢复”的问题已经破案：真实根因是 `render scale` 初始化把 `DPR` 倍率值错当成百分比处理，而不是后处理抗锯齿本身；当前已修正初始化换算，并在页面恢复可见性时强制重同步画布分辨率
