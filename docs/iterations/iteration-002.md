...
- 用户已通过 SSH 端口转发实际打开试页；结论是当前原始 `rgb PLY` 在浏览器里可流畅交互，但模型底部存在大量噪声高斯，且整体体积远大于有效区域
- 当前主观默认预览组合已收敛为：`crop-p05-p995 + 14x`

## 最新空间裁切分析

### 2026-03-23

- 已新增脚本：`scripts/gaussian_ply_tools.py`
- 已生成空间摘要：`outputs/iteration-002/analysis/export-rgb-summary.json`
- 当前 `rgb PLY` 的原始 bbox 为：`x = 77.32`、`y = 98.07`、`z = 55.33`
- 但分位数显示主体高斯实际集中在很小的范围内：`x` 约在 `[-0.94, 1.03]`、`y` 约在 `[-0.75, 0.79]`、`z` 约在 `[-0.49, 0.11]`
- 仅按三轴 `p0.5-p99.5` 做保守 box 裁切，可保留约 `97.49%` 的高斯：`4632969 / 4752215`
- 已产出 box 裁切候选：`outputs/iteration-002/crop-p05-p995/splat.ply`
- 已产出 `z` 下界裁切候选：`outputs/iteration-002/crop-zmin-p005/splat.ply`
- 仅做 `z >= -0.487185` 的底部裁切，仍可保留约 `99.50%` 的高斯：`4728453 / 4752215`
- 从字节体积看，裁切后的文件下降不算大：原始 `280381158` bytes，`z` 裁切约 `278979200` bytes，box 裁切约 `273345821` bytes
- 但按 `exp(scale_0 + scale_1 + scale_2)` 计算的“尺度体积代理”观察，`z` 轴最低 `0.5%` 的高斯已经占到了约 `99.93%` 的尾部体积代理
- 这说明当前主要问题不是“离群高斯很多”，而是“少量极大尺度离群高斯在视觉上极脏、极占体积范围”
...
- 当前真实浏览器观察已经给出更清楚的优化方向：优先验证底部 `z` 尾部裁切，而不是先重做导出格式
- 当前 viewer 方向已经足够稳定，下一 session 不再把微调 viewer 参数作为主线，而是转向 `GS` 模型本身的优化

## 本轮之后的最小下一步

- 保留 `crop-p05-p995 + 14x` 作为当前默认预览入口
- 下一 session 主线转向 `GS` 模型优化：优先考虑更精确的场景裁切、遮罩、素材组织与复训策略，而不是继续调 viewer
- 再根据模型优化结果决定是否需要继续投入更激进的裁切 / 剪枝或 `PLY -> ksplat`
